...
for i = 1:trials
     ...
     A2 = a2 / b2;
     A3 = a3 / b3;
     % x*log(1+y/x) can be written as -rel_entr(x,x+y), replace y with A3.
 
     cvx_begin
     cvx_solver Mosek
         variable y(3) %P
         variable x(3) %tau
         maximize(-rel_entr(x(1),x(1)+A1*y(1)) /log(2) +  -rel_entr(x(2),x(2)+A2*y(2)) / log(2) + -rel_entr(x(3),x(3)+A3*y(3)) / log(2))
         subject to
...
for j = 1:5
    ...
    for i = 1:trials
        ...
        cvx_solver Mosek
            variable y(3) %P
            variable x(3) %tau
            maximize((1/(1-A1/C1)*(rel_entr(A1*y(1)+x(1),C1*y(1)+x(1))+A1/C1*rel_entr(C1*y(1)+x(1),A1*y(1)+x(1))))/log(2) + (1/(1-A2/C2)*(rel_entr(A2*y(2)+x(2),C2*y(2)+x(2))+A2/C2*rel_entr(C2*y(2)+x(2),A2*y(2)+x(2)))) / log(2) + -rel_entr(x(3),x(3)+A3*y(3)) / log(2))
            subject to
