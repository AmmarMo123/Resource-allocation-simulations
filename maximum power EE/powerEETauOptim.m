...
for Pmax = 20:5:45
    ...
    for i = 1:trials
        ...
        while F(counter)>=delta         
            ...
            cvx_solver Mosek
                variable x(3) %tau
                maximize((-rel_entr(x(1)+A1,x(1)+A1+y1) - A1*(rel_entr((x(1)+A1)/y1,(x(1)+A1)/y1+1) + rel_entr((x(1)+A1)/y1+1,(x(1)+A1)/y1)))/log(2) + (-rel_entr(x(2)+A2,x(2)+A2+y2) - A2*(rel_entr((x(2)+A2)/y2,(x(2)+A2)/y2+1) + rel_entr((x(2)+A2)/y2+1,(x(2)+A2)/y2)))/log(2) + -rel_entr(x(3),x(3)+A3)/ log(2) - lambda(counter) * (circuitPower + y(1) + y(2) + y(3)))
                subject to
