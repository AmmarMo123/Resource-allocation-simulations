...
for j = 1:length(Gamma2)
    ...
    for i = 1:trials
        ...
        cvx_solver Mosek
            variable y(3) %P
            variable x(3) %tau
            maximize((Gamma1 * (1/(1-A1/C1)*(rel_entr(A1*y(1)+x(1),C1*y(1)+x(1))+A1/C1*rel_entr(C1*y(1)+x(1),A1*y(1)+x(1)))))/log(2) + (Gamma2(j) * (1/(1-A2/C2)*(rel_entr(A2*y(2)+x(2),C2*y(2)+x(2))+A2/C2*rel_entr(C2*y(2)+x(2),A2*y(2)+x(2))))) / log(2) + (Gamma3 * (1/(1-A3/C3)*(rel_entr(A3*y(3)+x(3),C3*y(3)+x(3))+A3/C3*rel_entr(C3*y(3)+x(3),A3*y(3)+x(3))))) / log(2))
            subject to
...
legend('Radar MI', 'ISaC MI', 'Comm MI');
