...
for i = 1:trials
     ...
     while F(counter)>=delta  && counter <= 20
         ...
         cvx_solver Mosek
             variable y(3) %power
             maximize((tau * log(1+ (A1 * y(1))))/ log(2) + (tau * log(1+ (A2 * y(2)))) / log(2) + (tau * log(1+ (A3 * y(3)))) / log(2) - lambda(counter) * (circuitPower + y(1) + y(2) + y(3))) 
             subject to
         ...
         lambda(counter) = ((tau * log2(1+(a1 * y(1))/(b1))) + (tau * log2(1+(a2 * y(2))/(b2))) + (tau * log2(1+(a3 * y(3))/(b3))))/(circuitPower + y(1) + y(2) + y(3));
