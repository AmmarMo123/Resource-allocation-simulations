...
for i = 1:trials
    ...
    while F(counter)>=delta && counter<=100
        cvx_begin
        cvx_solver Mosek
            variable x(3) %tau
            maximize(-rel_entr(x(1),x(1)+A1)/ log(2) +  -rel_entr(x(2),x(2)+A2)/ log(2) + -rel_entr(x(3),x(3)+A3)/ log(2) - lambda(counter) * (circuitPower + P + P + P))
            subject to
        ...
        lambda(counter) = ((x(1) * log2(1+(a1)/(b1*x(1)))) + (x(2) * log2(1+(a2)/(b2*x(2)))) + (x(3) * log2(1+(a3)/(b3*x(3)))))/(circuitPower + P + P + P);
...
for j = 1:5
    ...
    for i = 1:trials
        ...
        while F(counter)>=delta && counter<=100
             ...
             cvx_solver Mosek
                 variable x(3) %tau
                 maximize((-rel_entr(x(1)+A1,x(1)+A1+y1) - A1*(rel_entr((x(1)+A1)/y1,(x(1)+A1)/y1+1) + rel_entr((x(1)+A1)/y1+1,(x(1)+A1)/y1)))/log(2) + (-rel_entr(x(2)+A2,x(2)+A2+y2) - A2*(rel_entr((x(2)+A2)/y2,(x(2)+A2)/y2+1) + rel_entr((x(2)+A2)/y2+1,(x(2)+A2)/y2)))/log(2) + -rel_entr(x(3),x(3)+A3)/ log(2) - lambda(counter) * (circuitPower + P + P + P)) 
                 subject to
             ...
             counter = counter+1;
             F(counter) = cvx_optval;
             lambda(counter) = ((x(1) * log2(1+(a1)/((b1) + (c1*x(1))))) + (x(2) * log2(1+(a2)/((b2) + (c2*x(2))))) + (x(3) * log2(1+(a3)/(b3*x(3)))))/(circuitPower + P + P + P);
        end %end of while loop
